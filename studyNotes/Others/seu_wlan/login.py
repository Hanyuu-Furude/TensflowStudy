    ...
    while True:
        ...
        try:
            ...
            if flag==False:
                ...
                log = open('SeuLogin.log','a')
                log.write('['+time.strftime('%Y/%m/%d %H:%M:%S')+']'+" Connected.\n")
                log.close()
            time.sleep(600)
        except :
            print('['+time.strftime('%Y/%m/%d %H:%M:%S')+']'+" can't connect now.")
            if flag==True:
                flag=False
                log = open('SeuLogin.log','a')
                log.write('['+time.strftime('%Y/%m/%d %H:%M:%S')+']'+" can't connect now.\n")
                log.close()
